...
import Upbond, { UPBOND_BUILD_ENV, UpbondInpageProvider } from "@upbond/upbond-embed";
...
export class UpbondWalletConnector extends Connector {
  ready = !IS_SERVER;
  ...
  constructor(config: { chainList: Chain[]; options: Options }) {
    ...
    this.upbondInstance = new Upbond({ modalZIndex: config.options.modalZIndex });
  ...
  async connect(): Promise<Required<ConnectorData>> {
    try {
      ...
      if (this.upbondInstance.isInitialized && !this.upbondInstance.isLoggedIn) await this.upbondInstance.login();
  ...
  async switchChain(chainId: number) {
    try {
      ...
      const wagmiSelectedNetwork = this.chainList.find((chain) => chain.id === chainId);
  ...
  protected onAccountsChanged = (...accounts: unknown[]): void => {
    if (accounts.length === 0) {
      this.emit("disconnect");
    } else {
      window.parent.postMessage({ type: "UPBOND_SELECTED_ADDRESS", value: accounts[0] }, "*");
